...
export const SUPPORTED_LANGUAGES: LanguageConfig[] = [
  ...
  { code: 'de', name: 'Deutsch', urlPath: '/de/', sampleText: 'Bitcoin Leitfaden' },
  { code: 'fr', name: 'Français', urlPath: '/fr/', sampleText: 'Guides Bitcoin' },
  { code: 'es', name: 'Español', urlPath: '/es/', sampleText: 'Guías de Bitcoin' },
  ...
  { code: 'it', name: 'Italiano', urlPath: '/it/', sampleText: 'Guide Bitcoin' },
  { code: 'br', name: 'Português', urlPath: '/br/', sampleText: 'Guias de Bitcoin' },
...
export class LanguageSelectorPage extends BasePage {
  ...
  constructor(page: Page) {
    super(page);
    this.languageIcon = this.page.getByRole('img', { name: '99Bitcoins Languages' });
    this.languageDropdown = this.page.locator('.btc-header__lang-dropdown, .language-dropdown, [class*="lang"] ul');
  }

  async openLanguageDropdown(): Promise<void> {
    await this.languageIcon.click();
    // Wait for dropdown to be visible
    await this.page.waitForSelector('a[href*="/de/"], a[href*="/fr/"], a[href*="/es/"]', {
      state: 'visible',
      timeout: 5000
    });
  }
  ...
  async getAvailableLanguages(): Promise<string[]> {
    ...
    const languageLinks = this.page.locator('a').filter({
      ...
      hasText: /^(English|Deutsch|Français|Español|한국어|Türkiye|日本語|Italiano|Português|Nederlands|Norsk|Suomi|Русский|العربية)$/
    });
  ...
  async selectLanguage(languageName: string): Promise<void> {
    await this.openLanguageDropdown();
    await this.page.getByRole('link', { name: languageName, exact: true }).click();
  }
  ...
  async verifyLanguageDropdownVisible(): Promise<void> {
    ...
    await expect(this.page.getByRole('link', { name: 'English', exact: true })).toBeVisible();
    await expect(this.page.getByRole('link', { name: 'Deutsch', exact: true })).toBeVisible();
    await expect(this.page.getByRole('link', { name: 'Français', exact: true })).toBeVisible();
    await expect(this.page.getByRole('link', { name: 'Español', exact: true })).toBeVisible();
  }
  ...
  async switchToLanguageAndVerify(language: LanguageConfig): Promise<void> {
    ...
    await expect(this.page).toHaveURL(new RegExp(language.urlPath.replace('/', '\\/')));

    // Verify page contains translated content
    await expect(
      this.page.getByText(language.sampleText).first()
