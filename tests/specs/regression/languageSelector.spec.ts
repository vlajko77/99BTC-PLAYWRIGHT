...
test.describe('Language Selector', () => {
  ...
  test.describe('Language Dropdown', () => {
    ...
    test('dropdown shows correct language names', async ({ page }) => {
      ...
      const expectedLanguages = [
        ...
        '한국어', 'Türkiye', '日本語', 'Italiano',
        'Português', 'Nederlands', 'Norsk', 'Suomi',
        'Русский', 'العربية'
      ];
    ...
    test('each language link has correct href', async ({ page }) => {
      ...
      await expect(page.getByRole('link', { name: 'Deutsch', exact: true })).toHaveAttribute('href', '/de/');
      await expect(page.getByRole('link', { name: 'Français', exact: true })).toHaveAttribute('href', '/fr/');
      await expect(page.getByRole('link', { name: 'Español', exact: true })).toHaveAttribute('href', '/es/');
    });
  });

  test.describe('Language Switching', () => {
    test('switch to German and verify content is translated', async ({ page }) => {
      ...
      await expect(page.getByRole('link', { name: 'Krypto', exact: true })).toBeVisible();
      await expect(page.getByRole('link', { name: 'Prognosen', exact: true })).toBeVisible();
    });

    test('switch to French and verify content is translated', async ({ page }) => {
    ...
    test('switch to Spanish and verify content is translated', async ({ page }) => {
    ...
    test('switch to Japanese and verify content is translated', async ({ page }) => {
    ...
    test('switch back to English from another language', async ({ page }) => {
      ...
      await expect(page.getByRole('link', { name: 'Bitcoin Casinos', exact: true })).toBeVisible();
    });
  });
  ...
  test.describe('Language Persistence', () => {
    ...
    test('logo click from translated page goes to translated homepage', async ({ page }) => {
      await languagePage.selectLanguage('Deutsch');
      await expect(page).toHaveURL(/\/de\//);
  ...
  test.describe('Header Elements in Different Languages', () => {
    test('header maintains structure in German', async ({ page }) => {
      ...
      await expect(page.getByRole('link', { name: '99Bitcoins', exact: true })).toBeVisible();
      await expect(page.getByRole('img', { name: 'Search' })).toBeVisible();
      await expect(page.getByRole('img', { name: '99Bitcoins Languages' })).toBeVisible();
    });

    test('search icon is visible and clickable in translated version', async ({ page }) => {
      await languagePage.selectLanguage('Deutsch');
...
for (const lang of testLanguages) {
  test(`switch to ${lang.name} (${lang.code}) and verify URL`, async ({ page }) => {
