...
} from "../../data/aestheticShortcodes";

test.describe("Aesthetic Shortcodes", () => {
  test("CTA Button shortcode renders correctly", async ({ loginPage: _, postEditor, page }) => {
    await postEditor.createPostWithShortcode("CTA Button Test " + crypto.randomUUID(), CTA_BUTTON_SHORTCODE);
  ...
  test("Step by step guide shortcode renders with multiple items", async ({
    ...
    await postEditor.createPostWithShortcode("Step by Step Guide Test " + crypto.randomUUID(), STEP_BY_STEP_GUIDE_SHORTCODE);
  ...
  test("Countdown shortcode renders expired message when date has passed", async ({
    ...
    await postEditor.createPostWithShortcode("Countdown Test " + crypto.randomUUID(), COUNTDOWN_SHORTCODE);
  ...
  test("Checklist with green checkmarks shortcode", async ({ loginPage: _, postEditor, page }) => {
    await postEditor.createPostWithShortcode("Checklist Test " + crypto.randomUUID(), GREEN_CHECKMARKS_SHORTCODE);
  ...
  test("Pros and Cons shortcode renders sections", async ({ loginPage: _, postEditor, page }) => {
    await postEditor.createPostWithShortcode("Pros & Cons Test " + crypto.randomUUID(), PROS_AND_CONS_SHORTCODE);
  ...
  test("Verdict shortcode with rating and items", async ({ loginPage: _, postEditor, page }) => {
    await postEditor.createPostWithShortcode("Verdict Test " + crypto.randomUUID(), VERDICT_SHORTCODE);
  ...
  test("Star rating shortcode renders with label", async ({ loginPage: _, postEditor, page }) => {
    await postEditor.createPostWithShortcode("Star Rating Test " + crypto.randomUUID(), STAR_RATING_SHORTCODE);

    await expect(page.getByText("Review", { exact: true }).first()).toBeVisible();
  });

  test("Highlighted paragraph shortcode renders", async ({ loginPage: _, postEditor, page }) => {
    await postEditor.createPostWithShortcode("Highlighted Section Test " + crypto.randomUUID(), HIGHLIGHTED_PARAGRAPH_SHORTCODE);
  ...
  test("Key Takeaways shortcode with multiple items", async ({ loginPage: _, postEditor, page }) => {
    await postEditor.createPostWithShortcode("Key Takeaways Test " + crypto.randomUUID(), KEY_TAKEAWAYS_SHORTCODE);
  ...
  test("Multiple shortcodes combined in one post", async ({ loginPage: _, postEditor, page }) => {
    await postEditor.createPostWithShortcode("Combined Shortcodes Test " + crypto.randomUUID(), COMBINED_SHORTCODES_CONTENT);
